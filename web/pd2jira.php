...
if ($messages) foreach ($messages->messages as $webhook) {
  ...
  switch ($webhook_type) {
    case "incident.trigger":
      ...
      $data = array('fields'=>array('project'=>array('key'=>"$jira_project"),'summary'=>"$summary",'description'=>"A new PagerDuty ticket has been created.  {$trigger_summary_data}. Please go to $ticket_url to view it.", 'issuetype'=>array('name'=>"$jira_issue_type"),'Fix Version/s'=>array('id'=>"$jira_fixVersion")));
